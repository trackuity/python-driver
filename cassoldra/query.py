...
from cassoldra.util import unix_time_from_uuid1
from cassoldra.encoder import Encoder
import cassoldra.encoder
from cassoldra.protocol import _UNSET_VALUE
...
def tuple_factory(colnames, rows):
    ...
    .. versionchanged:: 2.0.0
        moved from ``cassoldra.decoder`` to ``cassoldra.query``
    """
    return rows
...
def ordered_dict_factory(colnames, rows):
    """
    Like :meth:`~cassoldra.query.dict_factory`, but returns each row as an OrderedDict,
...
class Statement(object):
    ...
    retry_policy = None
    """
    An instance of a :class:`cassoldra.policies.RetryPolicy` or one of its
...
ValueSequence = cassoldra.encoder.ValueSequence
"""
