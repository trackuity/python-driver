...
class ReadTimeout(Timeout):
    ...
    ``cassoldra.yaml`` with the ``read_request_timeout_in_ms``
...
class WriteTimeout(Timeout):
    ...
    This indicates that the replicas failed to respond to the coordinator
    node before the configured timeout. This timeout is configured in
    ``cassoldra.yaml`` with the ``write_request_timeout_in_ms``
    option.
    """

    write_type = None
    """
    The type of write operation, enum on :class:`~cassoldra.policies.WriteType`
    """
